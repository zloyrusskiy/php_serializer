defmodule PhpSerializerTest do
  ...
  test "unserialize defaults to ignoring excess data" do
    assert unserialize("N;i:0;") == {:ok, nil}
  end

  test "unserialize optionally returns excess data" do
    assert unserialize("N;i:0;", with_excess: true) == {:ok, nil, "i:0;"}
  end

  test "unserialize optionally fails with excess data" do
    assert unserialize("N;i:0;", strict: true) == {:error, "excess characters found"}
  end

  test "unserialize optionally fails with excess data and returns the excess it failed on" do
    assert unserialize("N;i:0;", strict: true, with_excess: true) ==
             {:error, "excess characters found", "i:0;"}
  end
